...
A private team pipeline for the LG Aimers / DACON competition (야구 투구 제어 성공 예측, `control_success` binary target, evaluated via Brier Skill Score). Built for Ubuntu 24.04 (WSL2), Python 3.11, CatBoost-based but written to be model-agnostic where practical (not verified beyond CatBoost).
...
1. Runs `code/train.py` as a subprocess → produces `open/temp/latest_model.pkl` (trained with a train/val split for evaluation).
2. Runs `code/test.py` as a subprocess → rebuilds the same val split, scores `latest_model.pkl` against `open/reference/best_model.pkl` (the current best), writes `NEW_BEST` or `KEEP_REF` to `open/temp/compare_result.txt`.
...
4. **Full retrain**: regardless of step 3's outcome, reloads whichever model is now `open/reference/best_model.pkl`, reads its `best_iteration_` (fallback 400), and retrains a fresh CatBoost model on the **entire** dataset (`is_train_split=False`, no held-out split) for `best_iteration_ + 10` iterations. This becomes `submit/model/final_retained_model.pkl` — i.e. the reference/eval model only decides *which config/hyperparams win*, but the actual submitted model is always retrained on all available data.

`submit/script.py` is the standalone inference entrypoint used by the competition server — it does **not** import from `code/`, it reimplements its own lightweight trackman feature merge (pitcher-level pitch-consistency std stats only, not the full situational join used in training). Keep this in sync manually if `code/train.py`'s feature set changes.
...
- `open/reference/best_model.pkl` — current best-scoring model, source of truth for `best_iteration_` used in full retrain.
- `open/former_model/` — auto-numbered backups of superseded latest/reference models, written by `dopip.py` every run.
- `submit/model/final_retained_model.pkl` — the actual artifact submitted to the competition server, always a full-data retrain.
